...
(function() {
    ...
    topWin.renderGalleryApp = function(container) {
        ...
        function renderGrid() {
            ...
            photos.forEach(p => {
                if (p.isUnlocked) {
                    // 使用 width:100%; height:0; padding-bottom:133.33% 强制锁定 3:4 比例，神仙来了也压不扁
                    g += `<div data-photo-url="${p.url}" data-photo-title="${p.title}" style="position:relative;width:100%;height:0;padding-bottom:133.33%;border-radius:12px;overflow:hidden;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.08);background:#e2e8f0;">
                    ...
                    g += `<div style="position:relative;width:100%;height:0;padding-bottom:133.33%;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);background:#1e293b;">
